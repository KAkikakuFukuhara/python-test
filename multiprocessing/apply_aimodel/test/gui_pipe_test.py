...
def resize_image(pipe:multiprocessing.Pipe) -> None:
    """ 画像をリサイズする
    """
    ...
    while True:
        ...
        pipe.send(result_image)
    print("process_exit")

def detection_object(pipe:multiprocessing.Pipe) -> None:
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.p = multiprocessing.Process(
            target=detection_object, args=(self.child_pipe, ), daemon=True
        )
        self.p.start()
