...
def valueToNdarray(v):
    n = np.ctypeslib.as_array(v.get_obj())  # Valueからndarrayに変換
    print(f"id_name ctypes : {id(n)}")
    return n
...
def ndarrayToValue(n):
    ...
    print(f"id_name value_object : {id(v.get_obj())}")
    print(f"id_name value : {id(v)}")
...
if __name__ == '__main__':
    # 適当な画像からndarrayの生成
    input_img = cv2.imread("./resize_map.png")
