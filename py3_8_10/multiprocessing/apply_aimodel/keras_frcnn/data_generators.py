...
def get_new_img_size(width, height, img_min_side=416):
    #　短辺が416になるようにそれぞれresizedを返す
    if width <= height:
        f = float(img_min_side) / width
        hub = f * height
        resized_height = int(hub)
        resized_width = img_min_side
    else:
        f = float(img_min_side) / height
        hub = f * width
        resized_width = int(hub)
...
class SampleSelector:
    ...
    def skip_sample_for_balanced_class(self, img_data):
        # class_count内にある情報から取り出したcurr_classと、ピックアップ中の学習用画像の中のgroundtruthのクラスが１つでも一致したら、
        # その画像での学習をスキップして次の画像を取り出す　ということらしい
...
def calc_rpn(C, img_data, width, height, resized_width, resized_height, img_length_calc_function):
    ...
    (output_width, output_height) = img_length_calc_function(resized_width, resized_height) # resnetの中にあるget_img_output_length のこと
    ...
    while True:
        ...
        for img_data in all_img_data:
            try:
                ...
                # BGR->RGBの変換　imreadで読み込んだ画像はBGR画像になってしまう
                x_img = x_img[:, :, (2, 1, 0)]
