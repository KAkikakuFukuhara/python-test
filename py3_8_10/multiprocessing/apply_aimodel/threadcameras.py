...
class ThreadCamera(Thread):
    ...
    def c_start(self):
        ...
        try:
            self.profile = self.p.start(self.config)
    ...
    def update_frames(self):
        ...
        if self.start_time is None:
            self.start_time = self.frames.get_timestamp()
            self.elapsed_time = 0
        else:
            self.elapsed_time = ( self.frames.get_timestamp() - self.start_time ) / 1000
        elaped_lock.release()
    ...
    def get_camera_angles(self) -> (float, float, float):
