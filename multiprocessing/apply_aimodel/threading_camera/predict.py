...
def predict_single_image(img_path, model_rpn, model_classifier, cfg, class_mapping):
    ...
    result = roi_helpers.rpn_to_roi(Y1, Y2, cfg, overlap_thresh=0.7)
    ...
    for jk in range(result.shape[0] // cfg.num_rois + 1):
        ...
        for ii in range(p_cls.shape[1]):
            # 分類確率がbbox_threshold未満のもの、背景のものはスキップ
            ...
            except Exception as e: # 背景ボックスにregrは与えられないので、背景ならこっちになる
                print(e)
                pass
...
def predict(args_):
    ...
    if os.path.isdir(path):
        for idx, img_name in enumerate(sorted(os.listdir(path))):
            if not img_name.lower().endswith(('.bmp', '.jpeg', '.jpg', '.JPG', '.png', '.tif', '.tiff')):
                continue
            print(img_name)
...
def parse_args():
    ...
    parser.add_argument('--path', '-p', default='./test_apple_images', help='image path')
